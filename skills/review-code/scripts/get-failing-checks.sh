...
get_repo_info() {
    ...
    if [[ -n "$upstream_repo" ]]; then
        OWNER="${upstream_repo%%/*}"
        REPO="${upstream_repo##*/}"
        printf '%b\n' "${BLUE}Detected fork workflow - checking upstream: $OWNER/$REPO${NC}"
    elif [[ -n "$origin_repo" ]]; then
        OWNER="${origin_repo%%/*}"
        REPO="${origin_repo##*/}"
    else
        printf '%b\n' "${RED}Error: Could not determine GitHub repository${NC}" >&2
        exit 1
    fi
}
...
parse_input() {
    ...
    if parse_workflow_url "$INPUT"; then
        MODE="run"
        printf '%b\n' "${BLUE}Detected workflow run URL${NC}"
        printf '%b\n' "${BLUE}Repository: $OWNER/$REPO${NC}"
        printf '%b\n' "${BLUE}Run ID: $RUN_ID${NC}"
        return
    fi
    ...
    # Invalid input
    printf '%b\n' "${RED}Error: Invalid input '$INPUT'${NC}"
...
handle_branch_mode() {
    ...
    if [[ -z "$head_sha" ]]; then
        printf '%b\n' "${RED}Error: Could not determine HEAD commit${NC}"
        exit 1
    fi

    printf '%b\n' "${BLUE}Checking CI status for branch '$current_branch' in $OWNER/$REPO${NC}"
    printf '%b\n' "${BLUE}Commit: ${head_sha:0:12}${NC}"
    echo
    ...
    if [[ "$total_runs" -eq 0 ]]; then
        printf '%b\n' "${YELLOW}No workflow runs found for this commit${NC}"
    ...
    if [[ "$in_progress_count" -gt 0 ]]; then
        printf '%b\n' "${YELLOW}$in_progress_count workflow(s) still running...${NC}"
    ...
    if [[ "$failed_count" -eq 0 ]]; then
        if [[ "$in_progress_count" -gt 0 ]]; then
            printf '%b\n' "${YELLOW}No failures yet, but $in_progress_count workflow(s) still running${NC}"
        else
            printf '%b\n' "${GREEN}✓ All $success_count workflow(s) passing${NC}"
        fi
        exit 0
    fi

    printf '%b\n' "${RED}Found $failed_count failing workflow(s):${NC}"
    ...
    _CLEANUP_DIRS+=("$log_dir")
    printf '%b\n' "${BLUE}Saving logs to: $log_dir${NC}"
    echo
    ...
    for run_id in $failed_run_ids; do
        local run_name
        run_name=$(echo "$runs" | jq -r --argjson id "$run_id" '.workflow_runs[] | select(.id == $id) | .name')
        local safe_name
        safe_name=$(tr ' /:()#' '_' <<< "$run_name" | tr -cd '[:alnum:]_-'); safe_name="${safe_name:0:50}"
        local log_file="$log_dir/${safe_name}_${run_id}.log"

        printf '%b\n' "${YELLOW}Fetching logs: $run_name (run $run_id)${NC}"
        if gh run view "$run_id" --repo "$OWNER/$REPO" --log-failed > "$log_file" 2>/dev/null; then
            local line_count
            line_count=$(wc -l < "$log_file" | tr -d ' ')
            if [[ "$line_count" -gt 0 ]]; then
                printf '%b\n' "  ${GREEN}✓${NC} Saved: $(basename "$log_file") ($line_count lines)"
                ((saved_count++))
            else
                printf '%b\n' "  ${YELLOW}(empty log)${NC}"
                rm -f "$log_file"
    ...
    if [[ "$saved_count" -gt 0 ]]; then
        printf '%b\n' "${GREEN}Logs saved to: $log_dir${NC}"
        echo
        echo "Files:"
        ls -la "$log_dir"
        echo
        ...
        local first_log=""
        for f in "$log_dir"/*.log; do [[ -f "$f" ]] && first_log="$f" && break; done
        if [[ -n "$first_log" ]] && [[ -f "$first_log" ]]; then
            printf '%b\n' "${BLUE}Preview of $(basename "$first_log"):${NC}"
            echo "----------------------------------------"
            tail -100 "$first_log"
            echo "----------------------------------------"
        fi
    else
        printf '%b\n' "${YELLOW}No log files were saved${NC}"
...
handle_workflow_run() {
    printf '%b\n' "${BLUE}Fetching workflow run logs...${NC}"
    echo
    ...
    if [[ -z "$run_info" ]]; then
        printf '%b\n' "${RED}Error: Could not fetch workflow run information${NC}"
        echo "Run ID: $RUN_ID"
    ...
    printf '%b\n' "${BLUE}Workflow: $run_name${NC}"
    printf '%b\n' "${BLUE}Status: $status${NC}"
    printf '%b\n' "${BLUE}Conclusion: $conclusion${NC}"
    echo
    ...
    if [[ "$conclusion" = "failure" ]]; then
        printf '%b\n' "${YELLOW}Fetching failed job logs...${NC}"
        if gh run view "$RUN_ID" --repo "$OWNER/$REPO" --log-failed > "$log_file" 2>/dev/null; then
            local line_count
            line_count=$(wc -l < "$log_file" | tr -d ' ')
            if [[ "$line_count" -gt 0 ]]; then
                ...
                printf '%b\n' "  ${YELLOW}(empty log - no failed jobs found)${NC}"
                rm -f "$log_file"
            fi
        else
            printf '%b\n' "  ${RED}✗ Failed to fetch logs${NC}"
            rm -f "$log_file"
        fi
    else
        printf '%b\n' "${YELLOW}Fetching all logs (run conclusion: $conclusion)...${NC}"
    ...
    if [[ -f "$log_file" ]]; then
        ...
        echo
        # Show preview
        printf '%b\n' "${BLUE}Preview of $(basename "$log_file"):${NC}"
        echo "----------------------------------------"
        tail -100 "$log_file"
        echo "----------------------------------------"
        echo
...
main() {
    ...
    if [[ -z "$PR_NUM" ]] && is_default_branch; then
        printf '%b\n' "${BLUE}On default branch - checking workflow runs directly${NC}"
        handle_branch_mode
        return
    fi
    ...
    printf '%b\n' "${BLUE}Checking CI status for PR #$PR_NUM in $OWNER/$REPO${NC}"
    ...
    if [[ -z "$checks_output" ]] || [[ "$checks_output" == *"no checks"* ]]; then
        printf '%b\n' "${YELLOW}No checks found for this PR${NC}"
        exit 0
    fi
    ...
    if [[ -z "$failing_checks" ]]; then
        ...
        printf '%b\n' "${BLUE}Context: $pr_dir${NC}"
        exit 0
    fi

    local fail_count
    fail_count=$(echo "$failing_checks" | wc -l | tr -d ' ')
    printf '%b\n' "${RED}Found $fail_count failing check(s):${NC}"
    echo
    ...
    if [[ -z "$failed_run_ids" ]]; then
        printf '%b\n' "${YELLOW}No GitHub Actions workflow logs available${NC}"
        echo
        ...
        printf '%b\n' "${GREEN}Check list saved to: $pr_dir/ci/failing-checks.txt${NC}"
        printf '%b\n' "${BLUE}Context: $pr_dir${NC}"
        exit 1
    fi

    printf '%b\n' "${BLUE}Saving logs to: $pr_dir/ci/${NC}"
    echo
    ...
    for run_id in $failed_run_ids; do
        ...
        if ci_logs_downloaded "$OWNER" "$REPO" "$PR_NUM" "$run_id"; then
            printf '%b\n' "${GREEN}✓ Already downloaded: run $run_id${NC}"
            ((saved_count++))
            continue
        fi

        local run_info
        run_info=$(echo "$runs" | jq --argjson id "$run_id" '.workflow_runs[] | select(.id == $id)')
        local run_name
        ...
        if gh run view "$run_id" --repo "$OWNER/$REPO" --log-failed > "$log_file" 2>/dev/null; then
            ...
            line_count=$(wc -l < "$log_file" | tr -d ' ')
            if [[ "$line_count" -gt 0 ]]; then
                printf '%b\n' "  ${GREEN}✓${NC} Saved: $(basename "$run_dir")/$(basename "$log_file") ($line_count lines)"
                ((saved_count++))
    ...
    echo
    if [[ "$saved_count" -gt 0 ]]; then
        printf '%b\n' "${GREEN}Logs saved to: $pr_dir/ci/${NC}"
        echo
        echo "Files:"
        ls -la "$pr_dir/ci/"
        echo
        ...
        for d in "$pr_dir/ci/"*/; do [[ -d "$d" ]] && latest_run_dir="$d" && break; done
        if [[ -n "$latest_run_dir" ]]; then
            local first_log=""
            for f in "$latest_run_dir"/*.log; do [[ -f "$f" ]] && first_log="$f" && break; done
            if [[ -n "$first_log" ]] && [[ -f "$first_log" ]]; then
                printf '%b\n' "${BLUE}Preview of $(basename "$latest_run_dir")/$(basename "$first_log"):${NC}"
                echo "----------------------------------------"
                tail -100 "$first_log"
                echo "----------------------------------------"
            fi
    ...
    echo
    printf '%b\n' "${BLUE}Context directory: $pr_dir${NC}"
    exit 1
}
